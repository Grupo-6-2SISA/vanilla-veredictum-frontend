document.addEventListener('DOMContentLoaded', () => {
    ...
    function createNotaRow(nota) {
        ...
        row.innerHTML = `
            ...
            <p class="nf-col-info">
                <button class="view-note-link">Ver mais</button>
            </p>
        `;
        return row;
    }
    ...
    function openModal(modalKey) {
        // Usa `display: flex` para modals que você quer centralizar
    ...
    // Botão de "Adicionar Nota Fiscal" no topo
    newAppointmentBtn.addEventListener('click', () => {
        openModal('add');
    });

    // Event listener unificado para os botões dentro das notas
    listItemsContainer.addEventListener('click', async (e) => {
        ...
        } else if (target.classList.contains('view-note-link')) {
            e.preventDefault();
            openModal('view');
    ...
    document.querySelectorAll('.modal-close-btn, .modal-close-delete-btn, .btn-cancel-delete').forEach(btn => {
        btn.addEventListener('click', (e) => {
            e.preventDefault();
            const modalElement = btn.closest('.modal, .modal-delete-note');
            if (modalElement) {
